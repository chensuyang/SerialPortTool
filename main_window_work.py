...
class main_window_work(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # 更新串口波特率选择列表
        self.init_baud_rate_combo_box(["2400", "4800", "9600", "57600", "115200", "230400"])
    ...
    def uart_read_callback(self):
        ...
        if g_current_opened_uart_info is not None:
            ...
            # 调用RX修饰器来转换串口的数据
            append_str = rx_decorator_handle.convert(uart_handle.read())

            # 如果有有效数据,则更新到界面
            if len(append_str):
                self.ui.RevDatatextEdit.append(append_str)
    ...
    def init_baud_rate_combo_box(self, baud_rate_list):
        ...
        self.ui.BaudRate_comboBox.addItems(baud_rate_list)
