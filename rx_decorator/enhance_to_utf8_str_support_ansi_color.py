...
NAME = "增强修饰器-转换到utf8编码的文本(支持ANSI着色)"
...
def ansi_color_str_convert(ansi_color_bytes):
    ...
    if m is not None:
        ...
        # 转为字符串
        out_str = str(ansi_color_bytes, encoding='utf-8', errors='ignore')
        return out_str, rgb_color
    else:
        return str(ansi_color_bytes, encoding='utf-8', errors='ignore'), (85, 85, 85)
...
def convert(bytes_data, series):
    ...
    if series:
        ...
        if bytes_data_idle_time_cnt > 50 and len(str_buf):
            print("超时打印")
            color = (85, 85, 85)
            out = str(str_buf, encoding='utf-8', errors='ignore')
            # 清空缓冲区
            str_buf = b""
